...
Changes we made from the offical gitlab-runner repo:
  ...
  - `common/version.go`
      ...
      - prometheus: `Name: "anka_gitlab_runner_version_info"`
  - `main.go`:
      - Commented out executor imports we don't need
      - Added Veertu author block and changed Usage to `"the Anka GitLab Runner"`
  - `commands/exec.go`:
      - Commented out executor imports we don't need
      - Added `_ "gitlab.com/gitlab-org/gitlab-runner/executors/anka"`
      - Commented out `Add self-volume to docker` code
  - `common/config.go`:
      - Added `AnkaConfig` struct
      - Deleted all Kubernetes and Docker functions
      - Deleted the loop that checks runner.Machine from `LoadConfig`
      - Deleted Docker, Parallels, and any other unused structs
      - Deleted executors we don't use from RunnerSettings
      - Deleted `DNSPolicy*` consts
  - `commands/config.go`:
      - `func getDefaultConfigFile`: `config.toml` -> `anka-config.toml` (allows multiple gitlab-runners on same host)
  - `common/config_test.go`:
      - Added `TestConfigParse` tests for anka since the default didn't work after pulling out docker
      - Deleted all kubernetes and docker tests
  - `Makefile`: 
      - Modified `NAME` ENV in  to be `anka-gitlab-runner`
      - Set `PACKAGE_NAME` to `gitlab-runner`
      - Deleted virtualbox and parallels lines from `development_setup`
  - `commands/register.go`: 
      - Deleted out `ask*` functions we don't use
      - Deleted `transformDockerServices` and anything else Docker related
      - We duplicated `askSSHLogin`, renamed it to `askAnkaSSHLogin`, then added it to the `exectorFns` so it prompts
      - Deleted executors we don't use from `exectorFns`
      - Added better prompt messages (s.Name was called description and made unregister confusing)
      - Changed `Invalid executor specified` message (logrus) to include the executor name
      - Added askAnka
  - `commands/register_test.go`:
      - Deleted Docker functions
  ...
  - `commands/unregister.go`: 
      - Added a failure if you don't include the name of the runner to unregister
      - Disabled all-runners struct option
  ...
  - `common/build_test.go`
      - Removed docker stuff (+ `TestJobImageExposed`)
      - Fixed `TestRetryPrepare` and `TestPrepareFailure` for PrepareRetries being 0 by default
  - `VERSION`
      - Added {gitlab runner version}/{anka executor version}
  - `build-and-*` bash scripts added
  - `docs` deleted
  - `network/trace.go` + `common/trace.go` + `common/network.go`: 
      - Added `IsJobSuccessful` function
  - `commands/builds_helper_test.go`
      - Deleted `TestBuildsHelperCollect` so tests pass
  - 
